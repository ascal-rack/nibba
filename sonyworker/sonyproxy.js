...
// Authorized hostnames allowed to embed this worker's player/API in an iframe.
const ALLOWED_HOSTS = [
    "nibbu.pages.dev",
    "nibbu.workers.dev",
...
// Framing headers so the authorized domains can embed the player without 403 / X-Frame-Options blocking.
    ...
    async fetch(request, env, ctx) {
        ...
        const checkAllowed = (str) => {
            ...
            try {
                ...
                return ALLOWED_HOSTS.some(h => raw.includes(h));
            }
        };

        // Detect iframe embedding directly from Sec-Fetch headers so iframe targets are never 403'd.
        const reqFetchDest = (request.headers.get('Sec-Fetch-Dest') || '').toLowerCase();
        const reqFetchMode = (request.headers.get('Sec-Fetch-Mode') || '').toLowerCase();
        const isIframeEmbed = reqFetchDest === 'iframe' || reqFetchDest === 'embed' || reqFetchDest === 'frame' || reqFetchMode === 'nested-navigate';

        const isFriendDomain = checkAllowed(origin) || checkAllowed(referer) || isIframeEmbed ||
        ...
        if (action === 'manifest' || action === 'manifest_url' || action === 'segment' || action === 'key') {
            ...
            // Accept iframe embeds, the worker's own origin, and any authorized host.
...
<body>
    ...
    return new Response(html, { headers: { 'Content-Type': 'text/html; charset=utf-8', ...IFRAME_ALLOW_HEADERS } });
}
