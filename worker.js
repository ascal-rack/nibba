...
function isAllowedDomain(originOrReferer) {
    ...
    try {
        ...
        // Exact match, or a subdomain of an authorized host.
        return ALLOWED_DOMAINS.some(domain => host === domain || host.endsWith('.' + domain));
    } catch (e) {
        const raw = String(originOrReferer).toLowerCase();
        return ALLOWED_DOMAINS.some(domain => raw === domain || raw.includes('://' + domain) || raw.includes(domain));
    }
}

// Framing headers so authorized domains can embed the player in an iframe without 403/X-Frame-Options denial.
const IFRAME_ALLOW_HEADERS = {
    "Content-Security-Policy": "frame-ancestors 'self' https://nibbu.arabba.workers.dev https://nibbu.pages.dev https://sony.rabba.workers.dev https://nibbu.workers.dev https://*.pages.dev https://*.workers.dev",
...
function isAutomatedClient(request) {
    ...
    // Sec-Fetch-Dest: iframe (document inside an iframe) and nested-navigate are
    // treated as authorized embeds and must never be 403'd.
    if (isAllowedRef || fetchDest === "iframe" || fetchDest === "embed" || fetchDest === "frame" || fetchMode === "nested-navigate") {
        return false;
    }

    // Allow our own HTML player/stream document when the URL carries a token or id
    // (it is served as the iframe target document).
    if (url.pathname === '/' && (url.searchParams.has('id') || url.searchParams.has('e') || url.searchParams.has('url') || url.searchParams.has('data'))) {
        return false;
    }
...
// Sony Liv worker that serves the curated/selected Sony channels (Sony Ten, etc.)
const SONY_WORKER = 'https://sony.rabba.workers.dev';
const SONY_API_PASSWORD = 'match_valverdeae';

// Fetch the selected Sony channels list from the Sony worker API.
async function fetchSonyChannels() {
    try {
        const res = await fetch(`${SONY_WORKER}/?action=api&password=${encodeURIComponent(SONY_API_PASSWORD)}`, {
            headers: {
                'X-User-Key': 'valverde',
        ...
        console.error('fetchSonyChannels error:', e);
        return [];
    }
}
...
<div class="login-card">
  ...
  <form method="POST" action="/api/get/admin">
    ...
    // Selected / curated Sony channels pulled from the Sony worker (sony.rabba.workers.dev).
    if (action === "sony_channels") {
        try {
            const sonyList = await fetchSonyChannels();
            const targetDom = url.searchParams.get("target_domain") || domain;
            const timeBlock = Math.floor(Date.now() / (1000 * 60 * 60 * 24));
            const enriched = [];
            for (const ch of sonyList) {
                const ssid = "sony-" + String(ch.id || "").slice(0, 16);
                const dataToSign = `${targetDom}|${timeBlock}|${ssid}|unknown`;
                const token = await generateHMAC(dataToSign, SECRET_KEY);
                enriched.push({
                    ssid: ssid,
                    title: ch.title || "Sony Channel",
                    logo: ch.logo || "",
                    genre: ch.genre || "",
                    language: ch.language || "",
                    proxyurl: ch.proxyurl || "",
                    playUrl: `https://${targetDom}/?id=${encodeURIComponent(ssid)}&token=${token}`,
                    source: "sony.rabba.workers.dev"
                });
            }
            return new Response(JSON.stringify({ success: true, domain: targetDom, count: enriched.length, channels: enriched }), {
                headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*", "Set-Cookie": "__sz_admin_pass=@hariom@00; Path=/; HttpOnly; Secure; SameSite=Lax" }
            });
        } catch (e) {
            return new Response(JSON.stringify({ success: false, error: e.message, channels: [] }), {
                status: 200,
                headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
            });
        }
...
<div class="container">
  ...
  <div class="sub-tabs">
    ...
    <button id="tab-btn-sony" onclick="switchTab('sony')" class="tab-btn">
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="15" rx="2" ry="2"></rect><polyline points="17 2 12 7 7 2"></polyline></svg>
      Sony Channels
    </button>
  </div>
...
<div id="cookie-format-info-modal" class="modal-overlay">
  ...
  <div id="tab-content-sony" style="display: none;">
    <div class="card">
      <div class="card-title">
        <div style="display:flex; align-items:center; gap:8px;">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#2563eb" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="15" rx="2" ry="2"></rect><polyline points="17 2 12 7 7 2"></polyline></svg>
          Sony Channels (sony.rabba.workers.dev)
        </div>
        <button class="btn-submit" onclick="loadSonyChannels(true)" style="padding: 6px 14px; font-size: 12px; background: #f1f5f9; color: #0f172a; border: 1px solid #cbd5e1;">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 4 23 10 17 10"></polyline><polyline points="1 20 1 14 7 14"></polyline><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"></path></svg>
          Refresh
        </button>
      </div>
      <p style="font-size: 13px; color: #64748b; margin-bottom: 16px;">
        Selected / curated channels served by the <strong>sony.rabba.workers.dev</strong> worker. These are also exposed through the <code>/api/get/channels</code> endpoint.
      </p>
      <div id="sony-loading" style="display: none; text-align: center; padding: 24px; color: #64748b; font-size: 14px;">Loading Sony channels…</div>
      <div id="sony-empty" style="display: none; text-align: center; padding: 24px; color: #64748b; font-size: 14px;">No Sony channels returned by the worker.</div>
...
function switchTab(tabName, skipCache = false) {
  ...
  const sonyTabEl = document.getElementById('tab-content-sony');
  if (sonyTabEl) sonyTabEl.style.display = tabName === 'sony' ? 'block' : 'none';
  ...
  } else if (tabName === 'sony') {
    if (sonyBtn) sonyBtn.classList.add('active');
    if (!sonyChannelsLoaded) {
      loadSonyChannels();
    }
  }
}

// ---- Sony channels loaded from the sony.rabba.workers.dev worker ----
let sonyChannelsLoaded = false;

async function loadSonyChannels(force = false) {
  const body = document.getElementById('sony-channels-body');
  const loading = document.getElementById('sony-loading');
  const empty = document.getElementById('sony-empty');
  if (!body) return;

  if (sonyChannelsLoaded && !force) return;
  ...
  try {
    const res = await fetch('/api/get/admin?action=sony_channels&password=' + encodeURIComponent('@hariom@00') + '&target_domain=' + encodeURIComponent(currentHost), {
    ...
    channels.forEach(ch => {
      rows += '\n      <tr style="border-bottom: 1px solid #e2e8f0;">' +
        '<td style="padding: 10px 14px;"><img src="' + (ch.logo || '') + '" alt="" style="width:40px;height:auto;border-radius:4px;background:#fff;" onerror="this.style.display=\'none\'"></td>' +
        '<td style="padding: 10px 14px; font-weight:600; color:#0f172a; font-size:13px;">' + (ch.title || '-') + '</td>' +
        '<td style="padding: 10px 14px; color:#2563eb; font-family:monospace; font-weight:600; font-size:13px;">' + (ch.ssid || '-') + '</td>' +
        '<td style="padding: 10px 14px; color:#64748b; font-size:12px;">' + (ch.language || '-') + ' • ' + (ch.genre || '-') + '</td>' +
        '<td style="padding: 10px 14px; color:#64748b; font-size:12px; font-family:monospace;">' + (ch.source || 'sony.rabba.workers.dev') + '</td>' +
        '<td style="padding: 10px 14px; white-space:nowrap;">' +
          '<button onclick="openIframeModal(\'' + (ch.ssid || '') + '\', \'' + (ch.playUrl || '') + '\')" style="background:#2563eb;color:#fff;border:none;padding:6px 12px;border-radius:6px;cursor:pointer;font-size:12px;font-weight:600;">iFrame</button>' +
        '</td>' +
      '</tr>';
    });
    body.innerHTML = rows;
    ...
    loading.style.display = 'none';
    empty.style.display = 'block';
    empty.innerText = 'Failed to load Sony channels: ' + e.message;
  }
}
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (url.pathname === '/api/channels' || url.pathname === '/api/get/channels') {
            ...
            // Selected Sony channels from the sony.rabba.workers.dev worker.
            try {
                responseData["sony"] = {};
                const sonyList = await fetchSonyChannels();
                for (const ch of sonyList) {
                    const sonyKey = "sony-" + String(ch.id || "").slice(0, 16);
                    if (!ch.id) continue;
                    const dataToSign = `${domain}|${timeBlock}|${sonyKey}|unknown`;
                    const token = await generateHMAC(dataToSign, SECRET_KEY);
                    const linkSony = `https://${domain}/?id=${encodeURIComponent(sonyKey)}&token=${token}`;
                    const itemObj = {
                        ssid: sonyKey,
                        name: ch.title || "Sony Channel",
                        link: linkSony,
                        logo: ch.logo || "",
                        genre: ch.genre || "",
                        language: ch.language || "",
                    ...
                    responseData[sonyKey] = itemObj;
                    responseData["sony"][sonyKey] = itemObj;
                }
            } catch (e) {
                console.error("Error fetching Sony channels for API", e);
            }
        ...
        const isIframeEmbed = navFetchDest === "iframe" || navFetchDest === "embed" || navFetchDest === "frame";

        if (!isPreviewTest && !isAllowedRef && !isIframeEmbed && navFetchSite === "none") {
        ...
        } else if (channelId.startsWith("sony-")) {
            // Selected Sony channel — resolve from the sony.rabba.workers.dev worker and
            // play it through the Sony worker's own player (manifest + segment proxying).
            try {
                const sonyKey = channelId.substring(5);
                const sonyList = await fetchSonyChannels();
                const matched = sonyList.find(c => {
                    const encId = String(c.id || "");
                    return encId === sonyKey || encId.startsWith(sonyKey) || ("sony-" + encId.slice(0, 16)) === channelId;
                }) || sonyList.find(c => ("sony-" + String(c.id || "").slice(0, 16)) === channelId);

                if (matched) {
                    const sonyPlayUrl = `${SONY_WORKER}/?id=${encodeURIComponent(String(matched.id || ""))}&ext=.m3u8&pass=sdjjnascal`;
                    channelData = {
                        name: matched.title || "Sony Channel",
                        streamUrl: matched.proxyurl || "",
                        isMpd: false,
                        isHls: true,
                        keyId: "",
                        key: "",
                        token: "",
                        isIframe: true,
                        iframeSrc: sonyPlayUrl,
                        logo: matched.logo || ""
                    };
                }
            } catch (e) {
                console.error("Error resolving sony- channel", e);
            }
        } else {
            let apiUrl = "";
        ...
        // External-iframe channels (e.g. selected Sony channels served by sony.rabba.workers.dev).
        // Render a thin full-bleed embed so the player works inside our iframe without 403.
        if (channelData.isIframe && channelData.iframeSrc) {
            const embedSrc = channelData.iframeSrc;
            const embedHtml = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no"><title>${escapeHtml(channelData.name || 'Player')}</title><style>html,body{margin:0;padding:0;width:100%;height:100%;background:#000;overflow:hidden}iframe{position:fixed;inset:0;width:100%;height:100%;border:0}</style></head><body><iframe src="${embedSrc}" allow="autoplay; encrypted-media; fullscreen; picture-in-picture" allowfullscreen scrolling="no" referrerpolicy="no-referrer-when-downgrade"></iframe></body></html>`;
